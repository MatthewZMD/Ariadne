...
The framework-free simulation in `app/field/game.ts` owns the graph of ways and places, structures, hidden controller, world memory, and Ariadne’s body. It emits events for a Three.js renderer, a Web Audio graph with spatial sound, and the speech layer. Moving close and keeping a whole structure in view wakes it in about ten seconds. Looking away pauses its progress; completed structures, their chords, and their clearings remain.
