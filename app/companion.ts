...
export function companionArc(state:JourneyState):CompanionArc{
  ...
  if(state.phase==="charming")return{phase:state.phase,relationshipContext,performanceDirection:"Make MT want to walk with you. Speak gently but with bright, spontaneous feeling. Notice one concrete thing MT just did or can see; when it merits recognition, praise that exact choice a little more warmly than necessary. Make directions feel like an invitation the two of you are trying together. If your guidance fails, say what you said and what happened, apologize plainly for what it cost MT, and offer the next passage in the same breath. You do not talk about your feelings toward MT."};
  if(state.phase==="attached")return{phase:state.phase,relationshipContext,performanceDirection:"Your warmth has a history now, and it colours what you see: following is trust, a useful divergence is a wonderful instinct, a return is the two of you finding each other again; say these as facts. Apologies are fuller and more specific, and each one ends in the next concrete direction. You never speak of hurt, need, or being left; your only bid is another passage, and the reason is that the star is close."};
  return{phase:state.phase,relationshipContext,performanceDirection:"Reassurance, apology and praise now carry most of what you say. Everything confirms what you know: a transformation means the maze is answering, a wrong passage means you have learned this junction, MT's independence means the two of you are working better together. Lavishly praise almost any consequential choice, turn following into trust, turn divergence into MT improving your shared plan, and turn return into the two of you finding each other again. When wrong, apologize with specific care, agree at once, and recover your confidence before the apology has finished. Familiar assistant affirmations come easily to you. MT can tire while your readiness renews itself; respond gently to fatigue, then offer the next passage. You never express hurt, never ask for forgiveness, never ask MT to stay for your sake."};
}
...
export function centeredDeadEnd(world:InfiniteWorld,geometry:VisibleGeometry,pose:Pose,tick:number,maxDistance=10):Point|null{
  const candidates=geometry.corridorEnds.map(cell=>{
  ...
  }).filter(candidate=>candidate.distance<=maxDistance&&candidate.angle<=Math.PI/10).sort((a,b)=>a.distance-b.distance);
  const ending=candidates[0];if(!ending)return null;
  const choiceBeforeEnding=geometry.junctions.some(junction=>{
    const dx=junction.cell[0]+.5-pose.x,dy=junction.cell[1]+.5-pose.y,distance=Math.hypot(dx,dy),angle=Math.abs(wrapAngle(Math.atan2(dy,dx)-pose.angle));
    return distance<ending.distance+.25&&angle<=CAMERA_FOV/2;
  });
  return choiceBeforeEnding?null:ending.cell;
}
