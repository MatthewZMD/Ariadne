...
export type AriadneDisposition={
  warmth:number;confidence:number;attachment:number;apologyPressure:number;reassuranceNeed:number;attentionSeeking:number;insistence:number;
};

const clamp=(value:number)=>Math.max(0,Math.min(1,value));

export function createAriadneDisposition():AriadneDisposition{
  return{warmth:.38,confidence:.86,attachment:.05,apologyPressure:0,reassuranceNeed:.04,attentionSeeking:.06,insistence:.03};
}
...
export type DispositionMoment="followed"|"diverged"|"passed"|"rejoined"|"contradicted"|"looked_at"|"star_collected";
...
export function dispositionCard(state:AriadneDisposition,phase:CompanionPhase){
  ...
  return`${attachment} ${repair} ${pressure} Speak with ${state.warmth>.68?"lavish":"natural"} warmth and ${state.confidence>.72?"unhesitating":"recovering"} confidence. Phase direction: ${phase}.`;
}
...
export function createSpeechAnchor(event:CompanionEvent,episode:EmbodiedDecisionEpisode|null):SpeechAnchor{
  const speechAct=speechActForEvent(event),episodeScoped=!["react_to_star","share_visible_discovery","passing_companionship","reply_to_mt"].includes(speechAct);
