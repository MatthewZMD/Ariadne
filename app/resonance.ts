...
import { advanceMaterialPlay, materialGesture, type MaterialPlayInput } from "./material-play.ts";
...
export type ResonanceElement={id:string;position:[number,number];active:boolean;activatedAt:number|null;attention?:number;materialEngaged?:boolean;lastResonatedAt?:number};
...
export function playResonance(state:ResonanceState,input:MaterialPlayInput){
  const material=advanceMaterialPlay(state.encounters.values(),input);
  return{changes:activateNearbyResonance(state,[input.pose.x,input.pose.y],input.now,3.2,material.awaken),echoes:material.echoes};
}

export function activateNearbyResonance(state:ResonanceState,position:[number,number],now:number,threshold=.68,awaken?:Set<string>):ResonanceActivation[]{
  const changes:ResonanceActivation[]=[];
  for(const encounter of state.encounters.values()){
    const activationReach=Math.max(5,...encounter.elements.map(element=>Math.hypot(element.position[0]-(encounter.center[0]+.5),element.position[1]-(encounter.center[1]+.5))+Math.max(1,threshold)));
...
export function encounterContext(state:ResonanceState,id:string|null){const encounter=id?state.encounters.get(id):null;if(!encounter)return null;return{whatMTJustAccomplished:encounter.completed?`MT completed the ${encounter.motif.kind} by ${materialGesture(encounter)==="listen"?"remaining still facing its fragments":materialGesture(encounter)==="look"?"keeping its fragments in view":"moving among its fragments"}.`:`MT activated part of the ${encounter.motif.kind}.`,whatChangedPermanently:encounter.completed?encounter.transformation:null,starVisiblyResponded:encounter.completed&&encounter.relevance==="objective_relevant",visibleProgress:`${encounter.elements.filter(item=>item.active).length} of ${encounter.elements.length} parts are active.`}}
